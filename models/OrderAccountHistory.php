...
class OrderAccountHistory extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['client_id', 'created_by', 'update_status', 'status_order_dukon', 'status_order_sklad', 'is_debt', 'is_delete', 'is_worker', 'large_price', 'fast_order', 'is_debtor', 'is_sent', 'is_vozvrat'], 'integer'],
    ...
    public function getInvoiceHtmlTextSklad($model, $id)
    {
        ...
        foreach ($warehouse as $model1) { 
            ...
            foreach ($orderProduct = ProductAccountHistory::find()->andWhere(['order_account_history_id' => $id])->andWhere(['brand_id' => $model1->brand_id])->andWhere(['or', ['vozvrat_order_id' => null], ['vozvrat_order_id' => 0]])->all() as $model1){
                $table .= '<tr style="">
    ...
    public function getInvoiceHtmlText($model, $id)
    {
        ...
        $vozvratSeparateRows = '';
        $vozvratSeparateIndex = 1;
        $vozvratSeparateCount = 0;
        $vozvratSeparateSum = 0;
        $vozvratSeparateRefund = 0;
        $table .= '<table style="width: 100%; border-collapse: collapse; margin-top: 15px;font-size:12px">
                    <tr style="">
                        <td style="height: 20px;text-align: center; width: 10px; border: 1px solid #000;"><b>№</b></td>
                        <td style="height: 15px;text-align: center; width: 15%; border: 1px solid #000;"><b>MODEL</b></td>
                        <td style="height: 15px;text-align: center; width: 15%; border: 1px solid #000;"><b>NOMI</b></td> 
                        <td style="height: 15px;text-align: center; width: 8%; border: 1px solid #000;"><b>SONI</b></td>
                        <td style="height: 15px;text-align: center; width: 8%; border: 1px solid #000;"><b>QOLDI</b></td>
                        <td style="height: 20px;text-align: center; width: 10%; border: 1px solid #000;"><b>TIP</b></td>                    
                        <td style="height: 15px;text-align: center; width: 10%; border: 1px solid #000;"><b>NARXI ($)</b></td>
                        <td style="height: 15px;text-align: center; width: 12%; border: 1px solid #000;"><b>UMUMIY NARXI ($)</b></td>
                    </tr>';
        ...
        foreach ($warehouse as $model1) { 
            ...
            foreach ($orderProduct = ProductAccountHistory::find()->andWhere(['order_account_history_id' => $id])->andWhere(['brand_id' => $model1->brand_id])->andWhere(['or', ['vozvrat_order_id' => null], ['vozvrat_order_id' => 0]])->all() as $model1){
                $vozvratRows = ProductAccountHistory::find()
                    ->where(['order_account_history_id' => $id])
                    ->andWhere(['brand_id' => $model1->brand_id])
                    ->andWhere(['product_category_id' => $model1->product_category_id])
                    ->andWhere(['size' => $model1->size])
                    ->andWhere(['type' => $model1->type])
                    ->andWhere(['type_sklad_id' => $model1->type_sklad_id])
                ...
                $vozvratLineSum = 0;
                $vozvratRowHtml = '';
                foreach ($vozvratRows as $vozvratRow) {
                    $rowCount = (int)$vozvratRow->count;
                    $rowSum = $vozvratRow->vozvrat_summa !== null
                        ? (float)$vozvratRow->vozvrat_summa
                        : $rowCount * (float)$vozvratRow->price;
                    $vozvratCount += $rowCount;
                    $vozvratLineSum += $rowSum;
                    $rowRefund = 0;

                    if ($vozvratRow->vozvratOrder) {
                        $vozvratOrderProductSum = (float)$vozvratRow->vozvratOrder->product_summ_dollar;
                        if ($vozvratOrderProductSum > 0) {
                            $rowRefund = ((float)$vozvratRow->vozvratOrder->all_summ_dollar * $rowSum) / $vozvratOrderProductSum;
                            $vozvratRefundSum += $rowRefund;
                        }
                    }

                    $vozvratDate = $vozvratRow->vozvratOrder
                        ? date('d.m.Y', strtotime($vozvratRow->vozvratOrder->cr_date_time ?: $vozvratRow->vozvratOrder->date))
                        : '';
                    $rowPrice = $rowCount > 0 ? $rowSum / $rowCount : (float)$vozvratRow->price;
                    $vozvratRowHtml .= '<tr style="color:red;">
                            <td style="height: 15px; border: 1px solid #000;"><b>&nbsp;'. $vozvratSeparateIndex . '</b></td>
                            <td style="height: 15px; border: 1px solid #000;"><b>&nbsp;'. $model1->brand->name . '</b></td>
                            <td style="height: 15px; border: 1px solid #000;"><b>&nbsp;'. $model1->productCategory->name.($model1->size!='0'?'&nbsp; (&nbsp;'. $model1->size.'&nbsp;)' : '') . '</b></td>
                            <td style="height: 15px; text-align: center; border: 1px solid #000;"><b>'. $vozvratDate . '</b></td>
                            <td style="height: 15px; text-align: center; border: 1px solid #000;"><b>'. $rowCount . '</b></td>
                            <td style="height: 15px; text-align: center; border: 1px solid #000;"><b>&nbsp;'. $model1->getTypeView($model1->type) . '</b></td>
                            <td style="height: 15px; text-align: center; border: 1px solid #000;"><b>'. Yii::$app->formatter->asDecimal($rowPrice, 2) . '</b></td>
                            <td style="height: 15px; text-align: center; border: 1px solid #000;"><b>'. Yii::$app->formatter->asDecimal($rowSum, 2) . '</b></td>
                        </tr>';
                    $vozvratSeparateIndex++;
                    $vozvratSeparateCount += $rowCount;
                    $vozvratSeparateSum += $rowSum;
                    $vozvratSeparateRefund += $rowRefund;
                }
                $vozvratSeparateRows .= $vozvratRowHtml;
                $vozvratProductSum += $vozvratLineSum;
                $leftCount = max(0, (int)$model1->count - $vozvratCount);
                $originalProductSum += (float)$model1->count * (float)$model1->price;
                $lineTotal = $leftCount * (float)$model1->price;
                $table .= '<tr style="">
                            <td style="height: 15px; border: 1px solid #000;"><b>&nbsp;'. $i . '</b></td>
                            <td style="height: 15px; border: 1px solid #000;"><b>&nbsp;'. $model1->brand->name . '</b></td>
                            <td style="height: 15px; border: 1px solid #000;"><b>&nbsp;'. $model1->productCategory->name.($model1->size!='0'?'&nbsp; (&nbsp;'. $model1->size.'&nbsp;)' : '') . '</b></td>
                            <td style="height: 15px; text-align: center; border: 1px solid #000;"><b>'. $model1->count . '</b></td>
                            <td style="height: 15px; text-align: center; border: 1px solid #000;"><b>'. $leftCount . '</b></td>
                            <td style="height: 15px; text-align: center;  border: 1px solid #000;"><b>&nbsp;'. $model1->getTypeView($model1->type) . '</b></td>
                            <td style="height: 15px; text-align: center;  border: 1px solid #000;"><b>'. $model1->price . '</b></td>
                            <td style="height: 15px; text-align: left;  border: 1px solid #000;"><b> = '. round($lineTotal,2) . '</b></td>
                        </tr>';
                $allCount = $allCount + $leftCount;
                $allSumm = $allSumm + $lineTotal;
                $i=$i+1;
            }
            // $table .= '<tr style="">
            //                 <td colspan="6" style="height: 20px; border: 1px solid #000;">&nbsp;</td>
        ...
        $table .= '
                    <tr style="">
                        <td colspan="5" style=";height: 20px; border: 1px solid #000;">&nbsp;<b>Jami</b></td>
        ...
        if ($vozvratSeparateRows) {
            $table .= '<div style="margin-top: 12px; color:red; font-size:13px;"><b>Vozvrat qilingan mahsulotlar</b></div>
                <table style="width: 100%; border-collapse: collapse; margin-top: 5px;font-size:12px">
                    <tr>
                        <td style="height: 20px;text-align: center; width: 10px; border: 1px solid #000;"><b>#</b></td>
                        <td style="height: 15px;text-align: center; width: 15%; border: 1px solid #000;"><b>MODEL</b></td>
                        <td style="height: 15px;text-align: center; width: 18%; border: 1px solid #000;"><b>NOMI</b></td>
                        <td style="height: 15px;text-align: center; width: 12%; border: 1px solid #000;"><b>SANA</b></td>
                        <td style="height: 15px;text-align: center; width: 8%; border: 1px solid #000;"><b>SONI</b></td>
                        <td style="height: 15px;text-align: center; width: 10%; border: 1px solid #000;"><b>TIP</b></td>
                        <td style="height: 15px;text-align: center; width: 10%; border: 1px solid #000;"><b>NARXI ($)</b></td>
                        <td style="height: 15px;text-align: center; width: 12%; border: 1px solid #000;"><b>UMUMIY NARXI ($)</b></td>
                    </tr>
                    '.$vozvratSeparateRows.'
                    <tr>
                        <td colspan="4" style="height: 20px; border: 1px solid #000;">&nbsp;<b>Jami</b></td>
                        <td style="height: 15px; text-align: center; border: 1px solid #000;"><b>'. $vozvratSeparateCount . '</b></td>
                        <td style="height: 15px; border: 1px solid #000;"></td>
                        <td style="height: 15px; border: 1px solid #000;"></td>
                        <td style="height: 15px; text-align: center; border: 1px solid #000;"><b>'. Yii::$app->formatter->asDecimal($vozvratSeparateSum, 2) . '</b></td>
                    </tr>
                </table>';
        }
        ...
        if ($vozvratProductSum > 0) {
            $vozvratSummaryRows = '
                <tr>
                    <th nowrap style="text-align: left; width: 150px; color:red">Vozvrat mahsulot summasi ($):</th>
                    <td ><b style="color:red">'.Yii::$app->formatter->asDecimal($vozvratProductSum, 2).' $,</b></td>
                    <td style="width: 10px;"></td>
                    <th nowrap style="text-align: left;color:red">Mijozga qaytarilgan summa ($):</th>
                    <td><b style="color:red">'.Yii::$app->formatter->asDecimal($vozvratRefundSum, 2).' $</b></td>
                </tr>
                <tr>
                    <th nowrap style="text-align: left; width: 150px; color:red">Qarzdan ayrilgan summa ($):</th>
                    <td ><b style="color:red">'.Yii::$app->formatter->asDecimal($vozvratDebtDiscount, 2).' $,</b></td>
                    <td style="width: 10px;"></td>
                    <th nowrap style="text-align: left;color:#f59c1">Hisobga kiradigan summa ($):</th>
                    <td><b style="color:#f59c1">'.Yii::$app->formatter->asDecimal($debtProductSum, 2).' $</b></td>
                </tr>';
        }
      ...
      ';
        ...
        $text = str_replace ("{exchange_rate}", $model->exchange_rate , $text);
        $text = str_replace ("{total_debt_old}", Yii::$app->formatter->asDecimal($model->total_debt_old, 2), $text);
        $text = str_replace ("{all_total_debt}", Yii::$app->formatter->asDecimal($effectiveDebt,2), $text);
        $text = str_replace ("{total_debt}", Yii::$app->formatter->asDecimal($effectiveDebt,2) , $text);
        $text = str_replace ("{discount_amount}", $model->discount_amount?$model->discount_amount:'' , $text);
        $text = str_replace ("{sum_dollar}", $model->sum_dollar?Yii::$app->formatter->asDecimal($model->sum_dollar,2):'0' , $text);
        $text = str_replace ("{all_summ_dollar}", Yii::$app->formatter->asDecimal($effectivePaid,2) , $text);
        $text = str_replace ("{sum_som}", $model->sum_som?Yii::$app->formatter->asDecimal($model->sum_som,2):' ' , $text);
        $text = str_replace ("{sum_transfers}", $model->sum_transfers?Yii::$app->formatter->asDecimal($model->sum_transfers,2):'0' , $text);
        $text = str_replace ("{sum_cart}", $model->sum_cart?Yii::$app->formatter->asDecimal($model->sum_cart, 2):'0' , $text);
        $text = str_replace ("{all_product_sum}", Yii::$app->formatter->asDecimal($originalProductSum, 2) , $text);
        $text = str_replace ("{date}", $model->date , $text);
